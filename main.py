...
class ChatPlus(Star):
    ...
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        self.probability_min_limit = config.get("probability_min_limit", 0.05)
        self.probability_max_limit = config.get("probability_max_limit", 0.8)
        ...
        # 日志输出
        logger.info("=" * 50)
        logger.info("群聊增强插件已加载 - V2.0.4-lite（精简重构版）")
    ...
    async def on_group_message(self, event: AstrMessageEvent):
        ...
        try:
            ...
            if (not _raw_msg_str or not _raw_msg_str.strip()) and not _msg_components:
                logger.info(
                    f"🚫 [空消息过滤] 检测到平台产生的真空消息（发送者: {event.get_sender_name()}），已直接丢弃"
                )
                event.call_llm = True
                return
