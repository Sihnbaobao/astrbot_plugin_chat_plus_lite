...
- **删除独立 Web 面板**（web/ 目录整体移除）：不再单独监听端口，不再需要登录密码/防爬虫/暴力破解防护/访问日志等一整套安全机制——管理界面直接做进 AstrBot Dashboard 的**插件页**（Plugin Pages，需 AstrBot v4.25.3+，v4.26.0+ 侧边栏还有插件 WebUI 入口）
- **删除访问日志**：访问日志/审计/自动清理等随独立面板一并移除，插件不再记录任何 IP 访问信息
- **新增插件页「管理控制台」**（pages/control/，卡片式可视化排版）：
  - 🧩 **消息处理流水线**：5 个胶囊（概率筛选 → 指令&关键词 → @必回处理 → 读空气AI判断 → 回复生成），点击胶囊展开该环节配置，开关状态一目了然
  - 🃏 **功能卡片网格**：图片识别 / 转发解析 / 黑名单 / 记忆注入 / 表情包降权 / 戳一戳 / 防复读 / Smart并发 / 内容过滤——每张卡片显示启用状态与关键参数摘要，点击展开编辑
  - 📊 **运行状态**：概率状态会话数、Smart 批次快照数、处理中会话数
  - 📜 **提示词预览**：读空气判断提示词 / 回复生成提示词全文（与真实拼接逻辑一致，含回复倾向段落）
  - 💾 面板内直接保存（白名单 45 项配置，即时写入并生效，Smart 参数自动同步）
- **配置项清理**：96 → 75 项（移除 enable_web_panel 及 20 个 web_panel_* 安全/日志配置键）
- **后端**：register_web_api 注册 status / config/save / prompts 三个 API；AstrBot 原生配置页（_conf_schema.json）全部能力保留
- **✅ 验证**：13 个回归测试通过；py_compile / node --check 通过

### V2.0.7-lite (2026-08-15)

**🧠 读空气判断逻辑优化（人格社交倾向优先）+ 移除概率硬性限制**

- **🐛 修复「老是回复」**：判断提示词原为"不确定时倾向于回复"的默认积极导向，与沉默寡言型人格（如"无口""寡言""冷淡"）冲突。现改为**人格社交倾向最高优先级**——沉默寡言型人格默认不回复普通闲聊，只回直接@/提问/求助/关键词实质相关；健谈型人格正常参与
- **✅ 建议回复清单收紧**：只保留明确需要回应的情况（@/呼唤/提问/求助/接话/记忆追问）
- **❌ 不回复清单扩充**：新增寒暄水消息（"哈哈哈哈""嗯嗯""+1"等）、纯陈述感叹分享（无互动意图）、他人闲聊默认不接
- **⚙️ 新增配置「读空气回复倾向」**（decision_ai_reply_tendency）：persona（遵循人格，默认）/ reserved（更保守，普通闲聊一律不回）/ active（更积极）
- **🗑️ 移除概率硬性限制**（enable_probability_hard_limit / probability_min_limit / probability_max_limit）：重构后概率机制简化，该限制无实用价值；移除后概率不会再被意外钳制（如 initial=1 被 max=0.8 压到 80% 回复）
- **同步**：科技树「AI读空气决策」节点新增倾向配置；Web 提示词预览同步新判断提示词
- **✅ 验证**：13 个回归测试通过；配置项 98 → 96
...
- **问题**：回复请求的 prompt 仅含纯上下文与一行"请直接输出你的回复"，无任何防复读引导。在群聊场景下模型容易把对方消息中的原词提取为"核心内容"，生成「火锅啊，…」这类确认式复读开头，且同一条回复内易重复相同句式
- **修复**：回复 prompt 结尾新增一行轻量防复读引导——不约束说话风格与人格，仅避免复述对方措辞、固定句式开头、提及上下文标注
- **说明**：读空气判断提示词中的【防止重复】职责不同——它管"要不要回复"（历史已充分表达则返回 no），本次未改动
- **同步**：prompt-data.js 回复AI说明更新；✅ 13 个回归测试通过
...
- **🎨 科技树流程图重写** — flow-data.js 移除主动对话/私聊两条流水线及注意力/等待窗口/情绪/拟人/疲劳/错字/打字/频率/质量评分/回复密度/时间段等已删节点，仅保留新流程（接入过滤→触发检测→概率判定→内容处理→读空气→回复生成→回复后处理）
- **💬 提示词预览更新** — prompt-data.js 移除频率判断AI/主动对话AI条目；读空气AI预览同步重构后提示词；回复AI条目说明重构后不再注入行为指令（system_prompt 只含人格）
- **📊 图表清理** — charts.js 移除注意力分布/情绪状态/主动对话统计等图表及已删功能统计卡片（723→484 行）
- **🗂️ 会话管理清理** — session-mgr.js 移除注意力/主动对话/情绪/等待窗口/冷却/疲劳/密度/活跃度等标签页与卡片（1187→900 行）
- **🔌 后端 API 清理** — server.py 清理已删功能的统计/详情/状态字段；/api/data/attention|mood|proactive 返回空结构兼容；删除 _safe_get_* 空壳函数
- **🧹 杂项** — api.js 删除 3 个已删功能 API 封装；app.js/config-editor.js/tech-tree.js/tech-tree.css 文案与样式清理
- **✅ 验证** — 全部 JS 文件 node --check 通过；13 个 Python 回归测试通过；前端无已删功能残留

### V2.0.1-lite (2026-08-15)

**精简重构版修复与兼容性更新**（派生自 V1.2.3.hotfix.2，重构：Sihnbaobao）

- **🎯 V2.0.0-lite 精简重构（初始发布）** — 重构核心理念：插件只决定"要不要回复"，不决定"说什么"；回复生成只传人格 + 纯上下文，不再注入任何行为指令（删除约 100 行 SYSTEM_REPLY_PROMPT 及情绪/注意力/主动对话等全部注入）。main.py 14493→5918 行、配置项 349→94 项；保留概率筛选/关键词/@必回/读空气/图片/转发/黑名单/时间戳标注/Web 面板/戳一戳/Smart 并发/livingmemory 记忆注入
- **🐛 修复：AstrBot v4.27.x 兼容性** — EventMessageType 改用 filter.EventMessageType 导入（v4.27.x 的 astrbot.api.event 包不再导出该符号），与 AstrBot 内置插件写法保持一致
- **🐛 修复：配置面板提供商选择器** — decision_ai_provider_id / image_to_text_provider_id 添加 _special=select_provider 元数据，AstrBot v4.27+ 面板据此渲染已有提供商下拉选择器
- **🎨 配置页清理** — 移除全部 _gcp_section_* 分区标题键（会被面板渲染成可编辑输入框导致页面杂乱），配置项保持 94 项
