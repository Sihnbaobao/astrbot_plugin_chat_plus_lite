...
- **🎯 V2.0.1-lite 精简重构（初始发布）** — 重构核心理念：插件只决定"要不要回复"，不决定"说什么"；回复生成只传人格 + 纯上下文，不再注入任何行为指令（删除约 100 行 SYSTEM_REPLY_PROMPT 及情绪/注意力/主动对话等全部注入）。main.py 14493→5918 行、配置项 349→94 项；保留概率筛选/关键词/@必回/读空气/图片/转发/黑名单/时间戳标注/Web 面板/戳一戳/Smart 并发/livingmemory 记忆注入
