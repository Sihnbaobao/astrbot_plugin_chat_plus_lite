...
class WebPanelServer:
    ...
    def _get_legacy_data_dirs(self) -> list[Path]:
        ...
        try:
            ...
            legacy_named = Path(
                StarTools.get_data_dir("astrbot_plugin_chat_plus_lite")
            ).resolve()
...
<h1>访问出错</h1><p>{safe_reason or "请联系管理员。"}</p></div></body></html>"""
    ...
    def _get_config_file_path(self) -> str:
        ...
        try:
            ...
            return os.path.join(
                config_dir,
                "astrbot_plugin_chat_plus_lite_config.json",
            )
        except Exception as e:
    ...
    async def _do_deferred_reload(self, host, port, dbc):
        ...
        try:
            ...
            async with _aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    reload_url,
                    json={"name": "astrbot_plugin_chat_plus_lite"},
    ...
    async def _build_session_detail(self, session: str):
        ...
        detail["cooldowns"] = []
        detail["pending_cooldowns"] = []
        detail["reply_density"] = {}
        detail["conversation_activity"] = {}
        detail["fatigue_blocks"] = []
