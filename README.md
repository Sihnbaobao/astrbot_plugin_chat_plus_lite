...
群聊默认开启，`group_reply_scope=ambient` 时保留普通群消息读空气。插件只会在明确回复其他用户、只@其他人/全体且没有正文时提前跳过；带有正文的他人对话会交给判断模型，由人格决定是否以旁观者身份自然补充一句。纯图片/贴纸和低信息短句仍会按明显噪声处理。未被接受的消息仍可进入缓存，但缓存只是背景，不会自动制造对话目标。
