...
群聊默认开启，`group_reply_scope=ambient` 时普通群消息可以进入参与判断。开放性群消息默认克制，有强烈且具体的个人切入点时可以展开回复；只有一个明确的第一人称补充时，也可能低打扰地插一句。明确回复其他用户的消息仍需人格自己的独立补充，且 side 参与保持强兴趣门槛。@、点名和关键词只提高注意力，不保证回复。纯图片/贴纸和低信息短句仍会按明显噪声处理。未被接受的消息会标记为 observation-only，不会自动制造对话目标；open/side 通过结果还受 45 秒、10 分钟、4 次的默认预算限制。
...
ambient 模式允许普通群消息进入判断。Persona 对当前话题有强烈且具体的个人连接时可以展开补充；即使只是较弱兴趣，只要有明确的第一人称切入点，也可能低打扰地说一句。open/side 回复还受参与预算限制。
