...
[![Version](https://img.shields.io/badge/version-0.0.11-blue.svg)](https://github.com/Sihnbaobao/astrbot_plugin_chat_plus_lite)
...
> 当前版本：0.0.11。插件元数据兼容 AstrBot >= 4.11.0；插件页管理控制台需要支持插件页的 AstrBot 版本，建议 4.25.3+。

## 功能概览

- 群聊读空气：默认观察普通群聊，但开放话题不是默认插话入口；模型需要判断当前 Persona 是否有具体、自然且足够强的参与理由。
...
插件不替换 AstrBot 当前人格，也不把旧版的情绪、注意力、主动对话等内部状态重新注入系统提示。DecisionAI 只决定是否继续正式回复流程，不把自己的思考过程传给正式回复模型；正式请求使用当前人格、消息上下文，以及涉及其他群友时的最小对象边界提示。reply_ai_extra_prompt 也只影响正式回复生成，不会被保存为用户历史正文。
...
群聊默认开启，`group_reply_scope=ambient` 时保留普通群消息读空气。开放性群消息默认不插话，只有当前 Persona 有具体、强且自然的个人切入点时才可能回复；明确回复其他用户的正文仍交给判断模型，由人格决定是否以旁观者身份补充一句。纯图片/贴纸和低信息短句仍会按明显噪声处理。未被接受的消息仍可进入缓存，但缓存只是背景，不会自动制造对话目标。
