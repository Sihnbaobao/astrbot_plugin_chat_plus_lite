...
不会仅凭旧历史认定 continuation。插件会先检查历史末尾是否确实是“当前发送者发言 -> 机器人紧接着回复”的相邻轮次；如果中间已经有其他群友发言，或历史尾部没有这组相邻消息，DecisionAI 必须把 continuation 判为 no，不能拿上午的机器人回复解释下午的新消息。下午的新消息仍可以被 Persona 当成一个全新的 open 话题来判断；明确 @ 机器人则照常按当前消息回应。单纯经过一段时间、期间没有其他消息时，不会被时间阈值强行切断。
