...
> ⚠️ 本文档描述的是旧版（V1.2.x）结构。当前分支为 refactor-lite（V2.2.0-lite）精简重构版：
...
> V2.2.0-lite 起，独立 Web 面板（web/ 目录）已整体移除，管理界面迁入 pages/control/ 插件页。
>
...
│
├── pages/                     # 🖥️ AstrBot 插件页（V2.2.0-lite 起）
...
## pages/control/ — AstrBot 插件页管理控制台（V2.2.0-lite 起）

> 原独立 Web 面板（web/ 目录）已于 V2.2.0-lite 整体移除，管理界面改为 AstrBot 插件页。
