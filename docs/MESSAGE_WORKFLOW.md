...
> **AI 全权主导（默认）**：`enable_random_probability_filter` 默认关闭——普通消息**不经过随机抽签**，直接进入 Phase 7 交给 AI 用人格判断是否回复（更像一个真实的人决定自己要不要说话）。只有当你希望恢复"随机概率"风格时，才打开该开关，此时下方的概率逻辑生效。
...
> 这是插件的**核心机制**：AI（以你的 LLM 人格为主角）判断"这条消息值不值得回、想不想回"。判断只决定"要不要回复"，不生成回复内容；最终回复由回复 AI 按你的人格正常生成。
>
> **人格第一立场**：判断注入当前会话人格（`decision_ai_include_persona` 默认开启）。判断提示词以人格本身的性格/兴趣/心情为准，仅保留"@/点名/提问/求助必回、别乱回他人对话/水话/被拒绝"等防乱回护栏，不再套用结论性的中性社交规则清单。
