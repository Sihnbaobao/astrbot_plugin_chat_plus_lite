...
> 群聊与私聊的正式回复都遵循当前会话人格；群聊和私聊使用不同的是否回复策略。DecisionAI 只返回是否继续的控制结果，正式回复阶段另行接收必要的群聊对象边界。
...
2. **版本号规则**（V主.次.补丁-lite）：
   - 修复 bug / 兼容性问题 → 补丁号 +1（如 V2.5.0-lite）
   - 新增功能 → 次版本号 +1（如 V2.5.0-lite）
...
4. **历史教训**（真实事故记录）：
   - V2.0.0-lite 发布后连续两次修复（EventMessageType 兼容、配置面板提供商选择器）均未升版本号，
     导致用户无法从版本号判断自己运行的是否为最新代码。自 V2.5.0-lite 起强制执行本规则。
