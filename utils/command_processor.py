...
from .probability_manager import (
    ProbabilityManager,  # noqa: F403  （与 main.py 保持一致）
)
...
class CommandMixin:
    ...
    async def gcp_reset(self, event: AstrMessageEvent):
        ...
        try:
            ...
            if not allowed:
                logger.info(
                    "【会话重置】用户 %s 未在白名单中，重置指令被忽略", sender_id
                )
                return
            try:
    ...
    async def _reset_session_data(self, event: AstrMessageEvent) -> None:
        ...
        try:
            ...
            try:
                k = str(chat_id)
                if (
                    isinstance(self.poke_trace_records, dict)
            ...
            try:
                await ContextManager.set_history_cutoff_async(chat_id)
        ...
        try:
            ...
            try:
                for cid in _all_chat_ids_for_cutoff:
                    try:
                        await ContextManager.set_history_cutoff_async(cid)
                    except Exception:
                        pass
                logger.info(
                    "【插件重置】已为 %s 个会话设置历史截止点",
            ...
            try:
                if self.image_description_cache:
                    await self.image_description_cache.clear_async()
            except Exception:
                pass
            try:
                await asyncio.to_thread(ContextManager._clear_all_custom_storage)
