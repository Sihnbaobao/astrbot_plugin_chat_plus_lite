...
def _filter_expired_cached_messages(
    cached_messages_raw: list,
    cache_ttl_seconds: int,
    max_cache_count: int,
    debug_mode: bool = False,
) -> list:
    """
    过滤过期的缓存消息（内联实现，原 ProactiveChatManager.filter_expired_cached_messages）

    Args:
        cached_messages_raw: 原始缓存消息列表
        cache_ttl_seconds: 缓存过期时间（秒）
        max_cache_count: 最大缓存条数
        debug_mode: 调试模式
    ...
    cache_ttl = max(60, int(cache_ttl_seconds))
    cache_max_count = max(1, int(max_cache_count))
    ...
    # 按 message_timestamp 排序，确保保留最新的消息
...
class MessageCacheManager:
    ...
    def get_cached_messages(
        ...
        # 过滤过期消息
        filtered_messages = _filter_expired_cached_messages(
            cached_messages,
