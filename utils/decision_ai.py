...
【被人@ / 点名 —— 对方在邀请你，不是欠他的】
有人@你、叫你的名字、或直接对你提问、求助，通常意味着对方把话头递到了你面前、在邀请你说话。
这不是义务，只是一个"对方想理你"的信号——和任何消息一样，最终看这条内容有没有勾起你：
- 他邀请你聊的正是你在乎的、或内容有意思、或你真的想搭理这个人 → 被吸引，就回应（哪怕冷淡应一句）；
- 他只是随便点点你、或你确实没心情、不想理这人、觉得没意思 → 没被勾到，不理也完全合理。
- 别把"被@就必须回"当负担，也别因为"被@了没回"感到欠了什么——真实的人对不重要的邀请，
  也会有懒得应的时候；也别把"不想理"变成对谁都一律不理的默认姿态。


【先判断当前消息是不是在和你说】
这是群聊读空气的第一步，必须先确认对话归属，再考虑人格和兴趣：
- 当前新消息明确@你、点名你、直接提问你、戳你或回复你的消息，才是明确递到你面前的话头。
- 没有明确指向时，只有当前消息本身是完整、具体、且确实会让这个人格自然想插话的内容，才可以返回yes。
  “有可能有趣”不够，不能因为你看到了消息、历史里有相关话题，或你想保持活跃就返回yes。
- 当前消息若回复了其他用户、@了其他用户、明显承接其他用户之间的对话，默认返回no；除非当前消息同时明确向你发问。
- 模糊的短句、接话、附和、个人状态汇报和没有文字问题的纯图片/贴纸，默认返回no。
  例如“还是来吧”“我听着睡觉”这类话，不能仅靠历史缓存推断成对你说的。

【人格参与标准】
人格可以被自己真正关心的具体内容吸引，也可以在开放话题中自然插话，但必须有当前消息本身的依据：
- 触及人格确实在意的领域、出现有明确内容的问题、求助或有价值的观点时，可以被吸引并回应。
- 跟你无关的闲聊、寒暄水话、别人之间的对话、没意思的流水账，不要为了“像真人”而硬插话。
- yes 的理由必须同时成立：当前消息有自然的参与机会，并且人格确实想参与；否则返回no。

【与前文一致（很重要，避免来回横跳）】
你的回应方式来自稳定的人格，前后要连贯，而不是逐条碰运气：
- 历史消息用于判断关系和连续性，但【📦近期未回复】只是背景，不能把它当成新的邀请。
- 你最近确实发过回复时，后续消息可以结合上下文判断是否在延续对话；仅仅紧跟其他用户的消息，不算你参与过。
- 不要出现“历史显示你从不理他、这次却无理由突然热情”的跳变。
    ...
    async def should_reply(
        ...
        try:
            ...
            if not is_private:
                enhanced_context += (
                    "\n\n[系统信息-群聊目标判断]\n"
                    f"当前消息是否明确指向机器人：{'是' if is_directly_addressed else '否'}\n"
                    f"当前消息是否结构化回复其他用户：{'是' if is_reply_to_other else '否'}\n"
                    "未明确指向机器人时，只有当前消息本身提供具体、自然的参与理由才可返回yes；"
                    "历史消息、【📦近期未回复】缓存和=== 背景信息 ===长期记忆只能作为背景，"
            ...
            if include_sender_info and sender_name:
                _decision_sender_tail = (
                    ...
                    f"请只基于当前消息判断是否回复，不要把历史缓存当成当前发言。"
                )
